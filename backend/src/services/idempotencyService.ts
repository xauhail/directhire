...
export class IdempotencyService {
   ...
   */
  static async getRecord(db: Pool, key: string) {
   ...
   */
  static async saveRecord(db: Pool, key: string, requestPath: string, status: number, body: string) {
    if (!db || !key) return;
    try {
      await db.query(
   ...
   */
  static async isWebhookProcessed(db: Pool, eventId: string): Promise<boolean> {
   ...
   */
  static async recordWebhook(db: Pool, eventId: string, eventType: string, payload: any) {
