...
const onboardingRoutes = async (server) => {
    ...
    server.post('/api/onboarding/parse-resume', async (req, reply) => {
        try {
            ...
            const parsed = geminiService_js_1.CareerAnalysisService.parseResume(text);
    ...
    server.post('/api/onboarding', async (req, reply) => {
        try {
            const data = req.body;
            const currentUser = await (0, auth_js_1.getAuthUser)(req);
          ...
          `, [
            ...
            const searchRes = await jobService_js_1.JobService.searchJobs({
    ...
    server.get('/api/onboarding/profile', async (req, reply) => {
        const currentUser = await (0, auth_js_1.getAuthUser)(req);
        const email = (req.query?.email || currentUser?.email || '').toLowerCase().trim();
        const userId = currentUser?.id;
        if (!auth_js_1.db || (!userId && !email)) {
            return reply.status(404).send({ error: 'Profile not found' });
        }
        try {
            const res = await auth_js_1.db.query('SELECT * FROM onboarding_profiles WHERE user_id = $1 OR LOWER(email) = $2 ORDER BY updated_at DESC LIMIT 1', [userId || '', email || '']);
