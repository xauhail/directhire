...
const toolRoutes = async (server) => {
    // Free Tool 1: ATS Resume Scanner & Keyword Matcher
    server.post('/api/tools/ats-score', async (req, reply) => {
        try {
            ...
            const parsedResume = await geminiService_js_1.GeminiService.parseResume(resumeText);
            ...
            const match = await geminiService_js_1.GeminiService.scoreJobMatch(parsedResume.skills, resumeText, jobTitle, jobDescription, jobSkills.length ? jobSkills : ['Technical Competency', 'Cross-functional Collaboration', 'Problem Solving']);
            return reply.send({
                success: true,
                score: match.score,
    ...
    server.post('/api/tools/ghost-detector', async (req, reply) => {
        try {
            ...
            const analysis = await geminiService_js_1.GeminiService.analyzeGhostJob(jobTitle, company || 'Unknown Company', description, Number(postedDaysAgo) || 14);
            return reply.send({
                success: true,
                ...analysis,
            });
    ...
    server.post('/api/tools/generate-outreach', async (req, reply) => {
        try {
            const body = req.body || {};
            ...
            const messages = await geminiService_js_1.GeminiService.generateOutreachMessages(candidateName, skills, company, roleTitle, hiringManagerName);
            return reply.send({
                success: true,
                messages,
    ...
    server.get('/api/tools/salary-benchmark', async (req, reply) => {
